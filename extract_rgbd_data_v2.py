...
SUNRGBDMeta2DBB = loadmat("SUNRGBDMeta2DBB_v2.mat")["SUNRGBDMeta2DBB"]
...
for imageId in range(len(SUNRGBDMeta)):
    ...
    data.rgbpath = data.rgbpath[17:]

    # Write point cloud in depth map
    rgb, points3d, depthInpaint, imsize = read3dPoints(data)
    valid_points_mask = ~np.isnan(points3d[:, 0])
    rgb = rgb[valid_points_mask, :]
    points3d = points3d[valid_points_mask, :]
...
def process_image(imageId):
    try:
        data = SUNRGBDMeta[imageId]
        data['depthpath'] = data['depthpath'][0][17:]
        data['rgbpath'] = data['rgbpath'][0][17:]
        ...
        with open(os.path.join(det_label_folder, txt_filename), 'w') as f:
            for j in range(len(data['groundtruth3DBB'])):
                box2d = data2d['groundtruth2DBB'][j]['gtBb2D'][0]
